...
import BackButton from "../components/BackButton"; // Сохранено, как в вашем проекте
...
const Input = styled.input`
  ...
  padding: 10px;
  margin-bottom: 0; /* Убираем margin-bottom здесь */
...
const RegistrationPage = () => {
  ...
  const fetchGroups = async () => {
    try {
      ...
      // В случае ошибки при получении групп, добавляем фиктивную группу для предотвращения падения
      // Этот блок оставлен, так как он был в последней версии, которая работала
      setGroups([{ id: "default", name: "Группа не загружена" }]);
      setFormData((prevData) => ({ ...prevData, group_id: "default" }));
    }
  };
  ...
  return (
    <Container>
      ...
      >
        <BackButton /> {/* ВАШ ОРИГИНАЛЬНЫЙ КОМПОНЕНТ */}
        <FormContainer>
          ...
          <Form onSubmit={handleSubmit}>
            ...
            {/* ИНТЕГРАЦИЯ ФУНКЦИОНАЛА ПАРОЛЯ: ТОЛЬКО ЭТОТ БЛОК ИЗМЕНЕН */}
            <Label>Пароль</Label>
            <PasswordWrapper>
              <Input
                // Динамическое изменение типа поля
              ...
              <ToggleIcon
                ...
                src={
                  showPassword
                    ? process.env.PUBLIC_URL + "/icons/eye-fill.svg"
                    : process.env.PUBLIC_URL + "/icons/eye-slash-fill.svg"
                }
              >
                {/* Иконка-переключатель */}
              </ToggleIcon>
            </PasswordWrapper>
            {/* КОНЕЦ ИНТЕГРАЦИИ ФУНКЦИОНАЛА ПАРОЛЯ */}
